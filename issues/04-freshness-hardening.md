...
Make `recall` keep itself current without re-reading the world, survive interruption, respect privacy, and be usable by a stranger from the README. This is the slice that turns the walking skeleton + full ingestion into something you can leave running and trust over time.
...
- **Incremental + batched backfill**: `recall index <dir>` becomes incremental — only changed/new files are touched. Backfill the full corpus in bounded batches with progress logging, never loading a whole file into memory (streaming parse from Slice 1 holds).
