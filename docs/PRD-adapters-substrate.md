...
- **Cursor**: code adapter over its SQLite store. The exact store must be re-sampled from real data before writing the issue: Cursor chat appears to live in per-workspace `state.vscdb` files as JSON blobs in a key-value table, which may be the very store an earlier framing called out of scope. Pin the table and key against a real database first; do not assume the location.
...
The `lore-setup` skill bundle is rewritten to cover non-JSONL formats, the two real tiers (write a code adapter, or push), the rewritten proof gate, and push as the immediate zero-setup path. The whole bundle is treated as one unit, consistent with the project rule that a skill is its entire folder.
...
Sequencing (the dependency DAG, corrected after the critic pass): (1) extract the shared write path; (2) the unified contract plus the tagged-union resume token and its schema migration; (3) rewrite the conformance harness into the record-stream round-trip gate. Only then, in parallel and each behind a real-data gate: openclaw, Cursor (after re-sampling to pin its real store), and Hermes. openclaw is not the early proof; it is just another code adapter. The conformance round-trip lands before the adapters, since it is how their correctness is proven.
