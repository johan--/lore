...
Ran `lore status`, exact searches, broadened search, `lore get`, and `lore context` against a live Lore repo store. The recall skill produced `/private/tmp/lore-live-skill-tests/recall-evidence-packet.json`, which passed `node skills/lore-recall/scripts/validate-evidence-packet.mjs`. The first exercise proved the drill-down loop worked on real ids but also exposed stale current-work recall: exact `UPD-004 package smoke` search returned zero hits because write sync was blocked by schema 5. After the schema-5 compatibility patch and rebuild, `lore search "UPD-004 package smoke" --source codex --project /repo/lore --since 2026-06-19T00:00:00.000Z --limit 3 --json` returned current 2026-06-19 evidence.
