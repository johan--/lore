...
Applied the verification skill to the workflow-pack itself after validator-only testing proved insufficient. Live tests against Lore found issues that static bundle checks missed: stale-window recovery recommended an impossible sync path for a newer store, `lore-brief` used an inconsistent private proposal shape, the main `skills/lore` entrypoint did not route to the new workflow skills, and CLI/package help under-described freshness/status. These were patched and covered by targeted tests/validators. A later schema-5 compatibility pass restored `lore sync codex` writes by porting content-hash migration/writer semantics and proved live status with `schemaVersion:5` / `supportedSchemaVersion:5`. Remaining risk: raw manual sync can overlap another sync and log `database is locked` for active files; the lock-protected Codex notify wrapper remains the safer routine freshness path.
